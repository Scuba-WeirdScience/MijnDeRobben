...
const _TW_SAFELIST = [
  ...
  'text-scuba-600',
  'dark:text-scuba-400',
  'hover:text-scuba-600',
  'dark:hover:text-scuba-400',
  'border-transparent',
];
...
export class VerzorgerTabComponent implements OnInit {
  ...
  loadVerzorgers(): void {
    ...
    this.verzorgerService.getVerzorgers(this.memberId()).subscribe({
      next: (list) => {
        this.verzorgers.set(list);
        this.loading.set(false);
      },
      error: () => {
        this.loading.set(false);
        this.toast.error('Verzorgers laden mislukt.');
      },
    });
  }
  ...
  createAndLink(): void {
    ...
    if (!this.newFirstName.trim()) {
      this.newFormError.set('Voornaam is verplicht.');
      return;
    }
    if (!this.newLastName.trim()) {
      this.newFormError.set('Achternaam is verplicht.');
      return;
    }
    if (!this.newEmail.trim()) {
      this.newFormError.set('E-mailadres is verplicht.');
      return;
    }

    this.saving.set(true);
    this.verzorgerService
      .createVerzorgerUser(
        this.memberId(),
        this.newEmail.trim(),
