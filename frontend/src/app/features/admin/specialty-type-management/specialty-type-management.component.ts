...
import { SpecialtyTypeService, SpecialtyType } from './specialty-type.service';
...
import {
  specialtyTypeSchema,
  type SpecialtyTypeForm,
} from '../../../shared/form-schemas';
import { LucideChevronDown } from '../../../shared/lucide-icons';

type SpecialtyTypeFormField = FieldTree<SpecialtyTypeForm>;
...
export class SpecialtyTypeManagementComponent implements OnInit {
  private readonly service = inject(SpecialtyTypeService);
  ...
  readonly items = signal<SpecialtyType[]>([]);
  readonly selectedOrg = signal<Organisatie | null>(null);
  readonly saving = signal(false);
  readonly showForm = signal(false);
  readonly editItem = signal<SpecialtyType | null>(null);
  readonly itemToDelete = signal<SpecialtyType | null>(null);
  readonly isExpanded = signal(true);

  readonly filtered = computed<SpecialtyType[]>(() => {
  ...
  readonly formModel = signal<SpecialtyTypeForm>({
    organisatie: '',
    naam: '',
    volgorde: 0,
  });
  readonly formState: SpecialtyTypeFormField;

  constructor() {
    this.formState = form<SpecialtyTypeForm>(this.formModel, specialtyTypeSchema as any);
  }
  ...
  private loadAll(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: () => { this.loading.set(false); this.toast.error('Kon specialiteiten niet laden.'); }
    });
  }
  ...
  // ── Form ──────────────────────────────────────────────────────────────────
  openForm(item: SpecialtyType | null = null): void {
  ...
  save(): void {
    ...
    req$.subscribe({
      next: () => {
        ...
        this.closeForm();
        this.loadAll();
        this.toast.success(existing ? 'Specialiteit bijgewerkt.' : 'Specialiteit toegevoegd.');
      },
      error: () => {
        this.saving.set(false);
  ...
  confirmDelete(item: SpecialtyType): void {
  ...
  onDeleteConfirmed(): void {
    ...
    this.service.delete(item.id).subscribe({
      next: () => {
        ...
        this.loadAll();
        this.toast.success('Specialiteit verwijderd.');
      },
